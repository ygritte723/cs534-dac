...
class ClipLoss(nn.Module):
    def __init__(
        ...
        # self.alpha = nn.Parameter(torch.tensor(1.0))  # Contrastive loss
        # self.beta = nn.Parameter(torch.tensor(1.0))   # MIL loss
        # self.gamma = nn.Parameter(torch.tensor(1.0))  # Negative loss
    ...
    def forward(
        ...
        # print("CLIPLoss is getting called--------------")
        # print(image_features.shape, text_features.shape, mil_texts_features.shape)
        # print(pos_that_have_negs)
        ...
        if self.args.mil_dense:
            loss_mil_dense = self.get_loss_mil_dense(
                logits_per_image_mil, logit_scale, logits_per_image_neg_mil, enable_K = 5
            )
    ...
    def get_loss_mil_dense(
        self, logits_per_image, logit_scale, logits_per_image_neg_mil=None, enable_K=5
    ):
        x = logits_per_image
        images_num = logits_per_image.shape[0]
        x = x.view(images_num, images_num, -1)  ## 32 images, 32, 10

        nominator = x * torch.eye(x.shape[0])[:, :, None].cuda() ### how positive matches contribute to the loss. The bigger the better
        nominator = nominator.sum(dim=1) ## [32, 10]
        ...
        if (
            ...
            # Hard Negative Mining: Select top-k hardest negatives per image
            k = enable_K # Number of hardest negatives to include
            # Compute the top-k logits along the negative dimension
            top_k_negatives, _ = torch.topk(x_neg, k=k, dim=2)  # Shape: bs, bs, k

            x_eye = x * torch.eye(x.shape[0])[:, :, None].cuda()
            x_eye = x_eye.sum(dim=1)
            
            x_eye_neg = top_k_negatives * torch.eye(top_k_negatives.shape[0])[:, :, None].cuda()
